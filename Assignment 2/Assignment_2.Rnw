...
\end{solution}
%%%%%%%%%%%%%%%%%%%%
\question Create a new scatter plot with \bf{all} of the \textt{Howell1} data, color-coded by male/female. Would the models from questions 5, 6, and 7 be appropriate to apply to the full data set? Explain. Connect to what we discussed about prediction in Lecture 3 of the Applied Regression class.
\begin{solution}
<<>>=
...
sp <- ggplot(data, aes(x=age, y=height)) + 
  geom_point(aes(color=sex)) +
