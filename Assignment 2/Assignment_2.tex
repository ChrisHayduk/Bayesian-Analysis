...
\hlstd{differences} \hlkwb{<-} \hlkwd{rep}\hlstd{(}\hlnum{NA}\hlstd{,} \hlkwd{length}\hlstd{(ages))}
\hlkwa{for}\hlstd{(i} \hlkwa{in} \hlnum{1}\hlopt{:}\hlkwd{length}\hlstd{(ages))\{}
  \hlstd{height} \hlkwb{<-} \hlstd{boys_height_chart[boys_height_chart}\hlopt{$}\hlstd{age} \hlopt{==} \hlstd{ages[i],]}\hlopt{$}\hlstd{`50th_perc`}
  \hlstd{heights[i]} \hlkwb{<-} \hlstd{height}

  \hlkwa{if}\hlstd{(i} \hlopt{!=} \hlnum{1}\hlstd{)\{}
    \hlstd{differences[i]} \hlkwb{<-} \hlstd{heights[i]} \hlopt{-} \hlstd{heights[i}\hlopt{-}\hlnum{1}\hlstd{]}
  \hlstd{\}}
\hlstd{\}}

\hlstd{boys_data} \hlkwb{<-} \hlkwd{data.frame}\hlstd{(}\hlstr{"age"} \hlstd{= ages,} \hlstr{"height"} \hlstd{= heights,} \hlstr{"diff"} \hlstd{= differences)}

\hlstd{boys_growth_rate} \hlkwb{<-} \hlkwd{mean}\hlstd{(boys_data}\hlopt{$}\hlstd{diff,} \hlkwc{na.rm} \hlstd{=} \hlnum{TRUE}\hlstd{)}
...
\hlstd{differences} \hlkwb{<-} \hlkwd{rep}\hlstd{(}\hlnum{NA}\hlstd{,} \hlkwd{length}\hlstd{(ages))}
\hlkwa{for}\hlstd{(i} \hlkwa{in} \hlnum{1}\hlopt{:}\hlkwd{length}\hlstd{(ages))\{}
  \hlstd{height} \hlkwb{<-} \hlstd{girls_height_chart[girls_height_chart}\hlopt{$}\hlstd{age} \hlopt{==} \hlstd{ages[i],]}\hlopt{$}\hlstd{`50th_perc`}
  \hlstd{heights[i]} \hlkwb{<-} \hlstd{height}
...
\hlstd{girls_data} \hlkwb{<-} \hlkwd{data.frame}\hlstd{(}\hlstr{"age"} \hlstd{= ages,} \hlstr{"height"} \hlstd{= heights,} \hlstr{"diff"} \hlstd{= differences)}

\hlstd{girls_growth_rate} \hlkwb{<-} \hlkwd{mean}\hlstd{(girls_data}\hlopt{$}\hlstd{diff,} \hlkwc{na.rm} \hlstd{=} \hlnum{TRUE}\hlstd{)}
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlstd{y} \hlkwb{<-} \hlstd{under_18}\hlopt{$}\hlstd{height}

\hlstd{intercept} \hlkwb{<-} \hlkwd{matrix}\hlstd{(}\hlkwd{rep}\hlstd{(}\hlnum{1}\hlstd{,} \hlkwd{length}\hlstd{(under_18}\hlopt{$}\hlstd{age)),} \hlkwc{ncol} \hlstd{=} \hlnum{1}\hlstd{)}

\hlstd{x_var} \hlkwb{<-} \hlkwd{matrix}\hlstd{(under_18}\hlopt{$}\hlstd{age,} \hlkwc{ncol} \hlstd{=} \hlnum{1}\hlstd{)}
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlstd{data}\hlopt{$}\hlstd{log_height} \hlkwb{<-} \hlkwd{log}\hlstd{(data}\hlopt{$}\hlstd{height)}
\hlstd{log_model} \hlkwb{<-} \hlkwd{lm}\hlstd{(log_height} \hlopt{~} \hlstd{age,} \hlkwc{data} \hlstd{= data)}

\hlkwd{summary}\hlstd{(log_model)}
\end{alltt}
\begin{verbatim}
## 
## Call:
...
\end{verbatim}
\begin{alltt}
\hlkwd{ggplot}\hlstd{(data,} \hlkwd{aes}\hlstd{(}\hlkwc{x}\hlstd{= age,} \hlkwc{y}\hlstd{=log_height))} \hlopt{+}
  \hlkwd{geom_point}\hlstd{(}\hlkwd{aes}\hlstd{(}\hlkwc{color}\hlstd{=sex))} \hlopt{+}
  \hlkwd{ggtitle}\hlstd{(}\hlstr{"Scatterplot with Log Model"}\hlstd{)} \hlopt{+}
  \hlkwd{labs}\hlstd{(}\hlkwc{x} \hlstd{=} \hlstr{"Age (years)"}\hlstd{,} \hlkwc{y} \hlstd{=} \hlstr{"Log(Height) (log cm)"}\hlstd{)} \hlopt{+}
  \hlkwd{scale_x_continuous}\hlstd{(}\hlkwc{breaks} \hlstd{=} \hlkwd{pretty}\hlstd{(data}\hlopt{$}\hlstd{age,} \hlkwc{n} \hlstd{=} \hlnum{7}\hlstd{))} \hlopt{+}
  \hlstd{theme.info} \hlopt{+}
  \hlkwd{stat_smooth}\hlstd{(}\hlkwd{aes}\hlstd{(}\hlkwc{y}\hlstd{=log_height),} \hlkwc{method} \hlstd{=} \hlstr{"lm"}\hlstd{,}
                  \hlkwc{formula} \hlstd{= y} \hlopt{~} \hlstd{x,} \hlkwc{size} \hlstd{=} \hlnum{1}\hlstd{,}
                  \hlkwc{color}\hlstd{=}\hlstr{"black"}\hlstd{)}
\end{alltt}
\end{kframe}
