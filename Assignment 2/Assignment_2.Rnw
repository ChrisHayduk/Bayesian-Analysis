...
differences <- rep(0, length(ages)-1)
for(age in ages){
  height <- boys_height_chart[boys_height_chart$age == age,]$`50th_perc`
  heights[i] <- height
  
  if(i != 1){
    differences[i-1] <- heights[i] - heights[i-1]
  }
  
  i <- i + 1
}

boys_growth_rate <- mean(differences)
...
differences <- rep(0, length(ages)-1)
for(age in ages){
  height <- girls_height_chart[girls_height_chart$age == age,]$`50th_perc`
  heights[i] <- height
...
girls_growth_rate <- mean(differences)
